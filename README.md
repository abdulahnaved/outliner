...
| **`data/`** | (Legacy) previously used for local SQLite; no longer required when using Neon Postgres. |
...
- `/report?scanId=...` — Saved report (Postgres, signed-in users).
...
**Auth / persistence (Next.js):** Postgres via `DATABASE_URL` (Neon recommended). Set `OUTLINER_AUTH_SECRET` (16+ chars) in production—see `.env.example`. The Python scanner is unchanged; only the Next app stores history.
